...
def ensure_pdf(job_id: str | None) -> tuple[str, bytes]:
    ...
    if job_id:
        ...
        # A tailored résumé that was never latexified falls back to the master
        # template, which is the same fallback the latexify node uses.
        if not tex.strip():
    ...
    key = cache_key(job_id, updated_at)
